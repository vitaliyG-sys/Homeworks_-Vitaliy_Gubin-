...
def test_log_with_filename() -> None:
    ...
    def addition_func(x: int | float, y: int | float) -> int | float:
        """Тестовая функция, складывающая 2 числа."""
        return x + y

    assert addition_func(1, 2) == 3
...
def test_log_without_filename() -> None:
    ...
    @log()
    def multiplication_func(x: int | float, y: int | float) -> int | float:
        """Тестовая функция, произведение двух чисел."""
        return x * y

    assert multiplication_func(5, 6) == 30
...
def test_log_console_logging(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    @log()
    def multiplication_func_2(x: int | float, y: int | float) -> int | float:
        """Тестовая функция, произведение двух чисел."""
        return x * y

    multiplication_func_2(1, 2)
...
def test_log_error(incorrect_data_x: int | bool | float, incorrect_data_y: Any, error_message: str) -> None:
    """4. Проверяет обработку исключений декоратора."""

    # Проверка работы декоратора с указанным параметром "filename".
    @log(filename="mylog.txt")
    def division_func(x: int | float, y: int | float) -> int | float:
        """Тестовая функция, деления двух чисел."""
        return x / y

    with pytest.raises(Exception, match=re.escape(error_message)):
        division_func(incorrect_data_x, incorrect_data_y)

    # Проверка работы декоратора без указанного параметра "filename".
    @log()
    def division_func_2(x: int | float, y: int | float) -> int | float:
        """Тестовая функция, деления двух чисел."""
        return x / y

    with pytest.raises(Exception, match=re.escape(error_message)):
        division_func_2(incorrect_data_x, incorrect_data_y)
